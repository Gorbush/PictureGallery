 ...
 */
@Component
public class FindSourcesByTimestamp implements PictureMatcher {
    ...
    public Collection<PictureInformation> find(PictureInformation source, String rootId, PictureGrade grade) {
        ...
        criteria.setTimestamp(source.getTimestamp());
//        criteria.setTimestamps(source.getTimestamps().stream().map(Timestamp::getStamp).collect(Collectors.toList()));
